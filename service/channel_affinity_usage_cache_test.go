...
// buildChannelAffinityStatsContextForTest 构造统计上下文。
//
// 注意：调用方必须使用**确定性唯一**的 ruleName / keyFingerprint（例如基于 t.Name()）。
// 原先这些测试用 time.Now().UnixNano() 生成"唯一"键，但在时钟粒度较粗的平台（如 Windows，
// 默认定时器精度约 15ms）上，相邻测试会拿到相同的时间戳，于是它们共用同一个计数条目，
// 断言随之失败——这是测试自身的缺陷，不是被测代码的问题。
